...
def write_reports(evidence: Path, rows: list[dict], selected: list[dict], selected_meta: list[dict], seed: int, threshold: float, rgb: dict) -> None:
    ...
    scoring = evidence / "scoring_utility"; scoring.mkdir(parents=True, exist_ok=True)
    (scoring / "README.md").write_text("# Organizer scoring utility\n\nThe organizer-provided scoring utility was not present in the supplied workspace and therefore no organizer-generated scoring output is included.\n", encoding="utf-8")
